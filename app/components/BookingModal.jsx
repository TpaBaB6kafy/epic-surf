      ...
      <AnimatePresence>{bookingModalUrl && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 md:p-6">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.9, y: 40 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 40 }} className="relative w-full max-w-5xl h-[85vh] bg-epicWhite rounded-[40px] overflow-hidden z-10 flex flex-col shadow-2xl border border-white/20">
            <div className="flex justify-between items-center p-6 border-b border-epicDark/10 bg-epicWhite text-epicDark font-bold uppercase text-[11px] tracking-wide leading-snug"><span>{title}</span><button onClick={() => setBookingModalUrl(null)} className="p-2 bg-epicMint rounded-full text-epicDark"><X size={18} /></button></div>
