...
test.describe("Home V2 approved fluid regression", () => {
  for (const route of routes) {
    ...
    test(`${route.path} keeps noindex/nofollow and locale-safe Header navigation`, async ({ page }) => {
  ...
  test("Home V2 stays out of sitemap and public production navigation", async ({ page }) => {
    ...
    for (const productionPath of ["/", "/ru"]) {
      const response = await page.goto(`${baseUrl}${productionPath}`, { waitUntil: "domcontentloaded" });
      expect(response?.ok()).toBeTruthy();
      await expect(page.locator("[data-home-v2-root]")).toHaveCount(0);
      await expect(page.locator('[data-home-v2-header="true"]')).toHaveCount(0);
